...
//Contar array
        //Entramos en la en el array KiR
        $completo=array();
        for($i=0;$i<count($this->ki_r);$i++) {
            $temp="";
            //Iteramos sobre cada KiR para hacer grupos de 6
            for($c=1, $j=0;$c<=8;$c++, $j+6) {
                $grupos=substr($this->ki_r[$i],$j,6);
                //Sacamos los datos que necesitamos para buscar en las tablas S.
                $fila=bindec(substr($grupos,0,1).substr($grupos,5,1));
                $columna=bindec(substr($grupos,1,4));

                $temp=$temp.str_pad(decbin($eses['s'.$c][$fila][$columna]),4,0, STR_PAD_LEFT);
            }
            array_push($completo, $temp);
        }
    $this -> eses = $completo;
    ...
    function tablaP () {
        $tabla=array(16,7,20,21,29,12,28,17,1,15,23,16,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25);
    }
}